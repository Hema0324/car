class Game {
  ...
  start() {
    ...
    cars = [car1, car2];
    car=createSprite(width/2+25,height-2000)
    ...
    this.addSprites(powerCoins, 3, powerCoinImage, 0.5);
    this.addSprites(powerCoins, 5, coin2Image, 0.1);
     this.addSprites(powerCoins, 2, coin3Image, 0.5);
 ...
 handleCollision(index) {
    if (cars[index - 1].collide(carGroup)) {
      if (this.leftKeyActive) {
        player.positionX += 100;
      } else {
        player.positionX -= 100;
